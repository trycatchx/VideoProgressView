...
public class PlayerUtils {
     ...
     * 获取安装应用的Intent
     */
    public static Intent getPlayByIntent(Context mContext, File file) {
        if (Build.VERSION.SDK_INT >= 24) {
            ...
            Intent chooser = Intent.createChooser(intent, "Play by");
    ...
    public static Bitmap songArt(Context context) {
        return BitmapFactory.decodeResource(context.getResources(), R.mipmap.ic_launcher);
    }

    public static boolean isAutoRotate(Context context) {
        try {
            int autoRotate = Settings.System.getInt(context.getContentResolver(), Settings.System.ACCELEROMETER_ROTATION);
            return autoRotate == 1;
